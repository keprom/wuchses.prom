...
class Billing extends Controller
{
    ...
    function firm_edit()
    {
        $sql = "SELECT firm.id,firm.dogovor,firm.address, firm.name,  firm.telefon, firm.rnn, firm.dogovor_date, firm.is_ip FROM industry.firm WHERE  firm.id=" . $this->uri->segment(3);
        $this->db->where('id', $this->uri->segment(3));
    ...
    function add_firm()
    {
        ...
        $this->db->order_by('name');
        $data['otrasl'] = $this->db->get('industry.firm_otrasl');
    ...
    {
        $this->check_billing_point($this->uri->segment(4));
    ...
    {
        $this->db->where("period_id>=" . $_POST['start_period_id'], "", false);
        $this->db->where("period_id<=" . $_POST['finish_period_id'], "", false);
        $data['nalog'] = $this->db->get("industry.report_to_nalogovaya");
    ...
    {/**/
        ...
        if (isset($_POST['nakladnaya']))
            $data['nakladnaya'] = "Накладная на отпуск запасов на сторону";
        else
            $data['nakladnaya'] = "СЧЕТ-ФАКТУРА";
        /**/
        $this->db->where('id', $_POST['firm_id']);
        ...
        $data['dog_hernya'] = $_POST['dog_hernya'];
    ...
    function schetoplata()
    {
        ...
        $this->db->where('id', $data['firm']->bank_id);
        $data['bank'] = $this->db->get("industry.bank")->row();
        $data['schet'] = !isset($_POST['schet']) ? " ОПЛАТУ" : "-ФАКТУРА";
...
//                $this->db->insert('industry.oplata', $data);
   ...
   left join industry.billing_point on 
    ...
    {
        $sql = "SELECT 
  tariff.id, tariff_value.value
FROM 
  industry.tariff_value 
 left join industry.tariff_period on tariff_period.id = tariff_value.tariff_period_id 
 left join industry.tariff on tariff_period.tariff_id=tariff.id 
 left join industry.period on period.end_date > tariff_period.data
WHERE 
   period.id = {$_POST['period_id']} AND 
  (((tariff.id =7) and 
  (tariff_period.data = (select max(tariff_period.data) as max 
	from industry.tariff
	left join industry.tariff_period on tariff.id = tariff_period.tariff_id
	where tariff.id = 7
	and tariff_period.data<=(select period.end_date 
			from industry.period
			where period.id={$_POST['period_id']})))) 
OR 
  ((tariff.id =8) and 
  (tariff_period.data = (select max(tariff_period.data) as max 
	from industry.tariff
...
where firm_id={$this->uri->segment(3)} and data_finish is null";
    ...
    public function narzad_po_ture()
    {
        switch ($_POST['is_finish']) {
            case '2':
                $this->db->where("data_finish is null");
                break;
            case '3':
                $this->db->where("data_finish is not null");
                $this->db->where("data_finish is not null");
                break;
            default:
                break;
        ...
        $data['report'] = $this->db->get("industry.narzad_po_ture")->result();

        if (isset($_POST['to_excel'])) {
            $this->export_to_excel("other_reports/narzad_po_ture/report", $data, 'Наряд-задание по ТУРЭ');
        } else {
            $this->load->view("other_reports/narzad_po_ture/report", $data);
        }
    }

    public function counter_by_type()
    {
		...
		public function export_rekvizit_schet()
    {
        ...
        if ($db) {
            ...
            if (!copy("c:/oplata/rschet.dbf", "c:/oplata/wses{$month}.dbf")) {
